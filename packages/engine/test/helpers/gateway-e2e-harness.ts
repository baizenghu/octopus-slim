...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `octopus-e2e-${name}-`));
  const configDir = path.join(homeDir, ".octopus");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OCTOPUS_SKIP_PROVIDERS: "1",
          OCTOPUS_SKIP_GMAIL_WATCHER: "1",
          OCTOPUS_SKIP_CRON: "1",
          OCTOPUS_SKIP_BROWSER_CONTROL_SERVER: "1",
          OCTOPUS_SKIP_CANVAS_HOST: "1",
          OCTOPUS_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
