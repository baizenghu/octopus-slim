...
const shouldBuild = (deps) => {
  if (deps.env.OCTOPUS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OCTOPUS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[octopus] ${message}\n`);
};

const runOctopus = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["octopus.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
