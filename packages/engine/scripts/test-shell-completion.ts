 ...
 * `octopus update`. Use it to verify the completion installation flow
...
\`octopus update\`, \`octopus doctor\`, and \`octopus onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Octopus Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`octopus completion --install\` later to enable.`));
    return;
  }
