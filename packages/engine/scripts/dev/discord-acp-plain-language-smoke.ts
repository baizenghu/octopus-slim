...
type DriverMode = "token" | "webhook" | "octopus";
...
function resolveStateDir(): string {
  const override = process.env.OCTOPUS_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.OCTOPUS_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".octopus");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver octopus] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Octopus spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|octopus> Driver transport mode (default: token)\n" +
    ...
    "  --octopus-bin <path>        Octopus CLI binary for driver=octopus (default: octopus)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  OCTOPUS_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  OCTOPUS_DISCORD_SMOKE_DRIVER\n" +
    "  OCTOPUS_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  OCTOPUS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  OCTOPUS_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  OCTOPUS_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  OCTOPUS_DISCORD_SMOKE_AGENT\n" +
    "  OCTOPUS_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  OCTOPUS_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  OCTOPUS_DISCORD_SMOKE_POLL_MS\n" +
    "  OCTOPUS_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  OCTOPUS_DISCORD_SMOKE_OCTOPUS_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.OCTOPUS_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.OCTOPUS_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.OCTOPUS_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.OCTOPUS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.OCTOPUS_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.OCTOPUS_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.OCTOPUS_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.OCTOPUS_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.OCTOPUS_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.OCTOPUS_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.OCTOPUS_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.OCTOPUS_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const octopusBin =
    resolveArg("--octopus-bin") || process.env.OCTOPUS_DISCORD_SMOKE_OCTOPUS_BIN || "octopus";
...
async function octopusCliJson<T>(params: { octopusBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.octopusBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`octopus ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await octopusCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    octopusBin: params.octopusBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "octopus") {
    return await readMessagesWithOpenclaw({
      octopusBin: params.args.octopusBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `octopus-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await octopusCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("octopus message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Octopus.`,
        diagnostics: {
          bindingCandidates: [
            {
