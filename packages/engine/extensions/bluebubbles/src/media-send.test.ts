...
import type { OctopusConfig, PluginRuntime } from "octopus/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): OctopusConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
