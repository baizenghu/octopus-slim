...
function resolveOctopusRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OCTOPUS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OCTOPUS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOctopusRoot(), "dist", "extensionAPI.js");
