import type {
  ...
  ReplyPayload,
  RuntimeEnv,
} from "octopus/plugin-sdk/mattermost";
import {
...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  if (slashEnabled) {
    try {
      ...
      // Use the *runtime* listener port when available (e.g. `octopus gateway run --port <port>`).
      // The gateway sets OCTOPUS_GATEWAY_PORT when it boots, but the config file may still contain
      // a different port.
      const envPortRaw = process.env.OCTOPUS_GATEWAY_PORT?.trim();
