...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__octopus_channel_id";
...
type InteractionCallbackConfig = Pick<OctopusConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "octopus-mattermost-interactions").update(botToken).digest("hex");
}
