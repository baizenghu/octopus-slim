...
export async function handleWhatsAppStart(host: OctopusApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OctopusApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OctopusApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OctopusApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OctopusApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OctopusApp): string {
...
function resolveGatewayHttpAuthHeader(host: OctopusApp): string | null {
...
function buildGatewayHttpHeaders(host: OctopusApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OctopusApp) {
...
export function handleNostrProfileToggleAdvanced(host: OctopusApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OctopusApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OctopusApp) {
  const state = host.nostrProfileFormState;
