...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as OctopusApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as OctopusApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as OctopusApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as OctopusApp);
    await loadToolsCatalog(host as unknown as OctopusApp);
    await loadConfig(host as unknown as OctopusApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as OctopusApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as OctopusApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as OctopusApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as OctopusApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as OctopusApp);
    await loadDevices(host as unknown as OctopusApp);
    await loadConfig(host as unknown as OctopusApp);
    await loadExecApprovals(host as unknown as OctopusApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as OctopusApp);
    await loadConfig(host as unknown as OctopusApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OctopusApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as OctopusApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__OCTOPUS_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OctopusApp, false),
    loadPresence(host as unknown as OctopusApp),
    loadSessions(host as unknown as OctopusApp),
    loadCronStatus(host as unknown as OctopusApp),
    loadDebug(host as unknown as OctopusApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OctopusApp, true),
    loadConfigSchema(host as unknown as OctopusApp),
    loadConfig(host as unknown as OctopusApp),
  ]);
}

export async function loadCron(host: SettingsHost) {
  const cronHost = host as unknown as OctopusApp;
  await Promise.all([
    loadChannels(host as unknown as OctopusApp, false),
