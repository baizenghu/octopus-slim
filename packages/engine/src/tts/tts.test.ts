...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.OCTOPUS_TTS_PREFS;
      process.env.OCTOPUS_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.OCTOPUS_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: OctopusConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
