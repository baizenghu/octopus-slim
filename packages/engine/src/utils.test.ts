...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("octopus-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("octopus-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("octopus-lid-a-", (first) => {
      withTempDirSync("octopus-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.octopus when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "octopus-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OCTOPUS_HOME over HOME", () => {
    vi.stubEnv("OCTOPUS_HOME", "/srv/octopus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/octopus-home"));
...
describe("shortenHomePath", () => {
  it("uses $OCTOPUS_HOME prefix when OCTOPUS_HOME is set", () => {
    vi.stubEnv("OCTOPUS_HOME", "/srv/octopus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/octopus-home")}/.octopus/octopus.json`)).toBe(
      "$OCTOPUS_HOME/.octopus/octopus.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OCTOPUS_HOME replacement when OCTOPUS_HOME is set", () => {
    vi.stubEnv("OCTOPUS_HOME", "/srv/octopus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/octopus-home")}/.octopus/octopus.json`),
    ).toBe("config: $OCTOPUS_HOME/.octopus/octopus.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/octopus")).toBe(path.resolve(os.homedir(), "octopus"));
  });
  ...
  it("prefers OCTOPUS_HOME for tilde expansion", () => {
    vi.stubEnv("OCTOPUS_HOME", "/srv/octopus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/octopus")).toBe(path.resolve("/srv/octopus-home", "octopus"));
