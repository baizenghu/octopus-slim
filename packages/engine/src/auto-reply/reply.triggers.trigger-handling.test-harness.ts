...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    octopusHome: process.env.OCTOPUS_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OCTOPUS_HOME", snapshot.octopusHome);
  restoreKey("OCTOPUS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OCTOPUS_STATE_DIR = join(home, ".octopus");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "octopus-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".octopus", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OctopusConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "octopus"),
