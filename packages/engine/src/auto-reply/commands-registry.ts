...
export function isCommandEnabled(cfg: OctopusConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OctopusConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OctopusConfig),
...
export function getCommandDetection(_cfg?: OctopusConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OctopusConfig) {
