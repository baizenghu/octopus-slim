...
export async function drainFormattedSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: OctopusConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: OctopusConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.OCTOPUS_TEST_FAST === "1") {
  ...
  const skillFilterChanged = !matchesSkillFilter(nextEntry?.skillsSnapshot?.skillFilter, skillFilter);
  const shouldRefreshSnapshot =
    skillFilterChanged || (snapshotVersion > 0 && (nextEntry?.skillsSnapshot?.version ?? 0) < snapshotVersion);
