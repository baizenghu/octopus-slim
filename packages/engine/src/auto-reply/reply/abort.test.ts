...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop octopus!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@octopus_bot", { botUsername: "octopus_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@octopus_bot", { botUsername: "octopus_bot" })).toBe(true);
