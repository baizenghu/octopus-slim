...
import { scheduleGatewaySigusr1Restart, triggerOctopusRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Octopus in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOctopusRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Octopus via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
