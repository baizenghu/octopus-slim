...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@octopus/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@octopus/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@octopus/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@octopus/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @octopus/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@octopus/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@octopus/plugin-alpha@1.2.3",
        installPath: "/tmp/octopus/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@octopus/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @octopus/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@octopus/plugin-alpha@latest",
      true,
      "/tmp/octopus/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@octopus/plugin-alpha@latest",
      installPath: "/tmp/octopus/extensions/alpha",
      version: "1.2.3",
