...
import type { OctopusConfig, ConfigFileSnapshot } from "../config/types.octopus.js";
...
vi.mock("../infra/octopus-root.js", () => ({
  resolveOctopusPackageRoot: vi.fn(),
}));
...
const { resolveOctopusPackageRoot } = await import("../infra/octopus-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/octopus-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OctopusConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/octopus-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOctopusPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("octopus-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveOctopusPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("octopus");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/octopus-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "octopus-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Octopus update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OctopusConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOctopusPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("octopus-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OCTOPUS_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.OCTOPUS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("octopus-update-wizard");
    await withEnvAsync({ OCTOPUS_GIT_DIR: tempDir }, async () => {
      setTty(true);
