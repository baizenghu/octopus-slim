    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["octopus memory status", "Show index and provider status."],
          ["octopus memory status --deep", "Probe embedding provider readiness."],
          ["octopus memory index --force", "Force a full reindex."],
          ['octopus memory search "meeting notes"', "Quick search using positional query."],
          [
            'octopus memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["octopus memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.octopus.ai/cli/memory")}\n`,
    );
