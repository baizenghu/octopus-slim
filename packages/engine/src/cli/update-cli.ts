...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Octopus and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["octopus update", "Update a source checkout (git)"],
        ["octopus update --channel beta", "Switch to beta channel (git + npm)"],
        ["octopus update --channel dev", "Switch to dev channel (git + npm)"],
        ["octopus update --tag beta", "One-off update to a dist-tag or version"],
        ["octopus update --dry-run", "Preview actions without changing anything"],
        ["octopus update --no-restart", "Update without restarting the service"],
        ["octopus update --json", "Output result as JSON"],
        ["octopus update --yes", "Non-interactive (accept downgrade prompts)"],
        ["octopus update wizard", "Interactive update wizard"],
        ["octopus --update", "Shorthand for octopus update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run octopus update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.octopus.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["octopus update status", "Show channel + version status."],
          ["octopus update status --json", "JSON output."],
          ["octopus update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.octopus.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
