...
export function registerBackupCommand(program: Command) {
  const backup = program
    .command("backup")
    .description("Create and verify local backup archives for Octopus state")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.octopus.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["octopus backup create", "Create a timestamped backup in the current directory."],
          [
            "octopus backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "octopus backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "octopus backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "octopus backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["octopus backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "octopus backup verify ./2026-03-09T00-00-00.000Z-octopus-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "octopus backup verify ~/Backups/latest.tar.gz --json",
