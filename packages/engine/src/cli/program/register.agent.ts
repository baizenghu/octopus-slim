...
${formatHelpExamples([
  ['octopus agent --to +15555550123 --message "status update"', "Start a new session."],
  ['octopus agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'octopus agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'octopus agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['octopus agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'octopus agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.octopus.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.octopus.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['octopus agents set-identity --agent main --name "Octopus" --emoji "🦞"', "Set name + emoji."],
  ["octopus agents set-identity --agent main --avatar avatars/octopus.png", "Set avatar path."],
  [
    "octopus agents set-identity --workspace ~/.octopus/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "octopus agents set-identity --identity-file ~/.octopus/workspace/IDENTITY.md --agent main",
