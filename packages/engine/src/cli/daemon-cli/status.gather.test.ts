...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OCTOPUS_STATE_DIR: "/tmp/octopus-daemon",
    OCTOPUS_CONFIG_PATH: "/tmp/octopus-daemon/octopus.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OCTOPUS_STATE_DIR ?? "/tmp/octopus-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OCTOPUS_CONFIG_PATH ?? `${stateDir}/octopus.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/octopus-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OCTOPUS_STATE_DIR = "/tmp/octopus-cli";
    process.env.OCTOPUS_CONFIG_PATH = "/tmp/octopus-cli/octopus.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.OCTOPUS_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
