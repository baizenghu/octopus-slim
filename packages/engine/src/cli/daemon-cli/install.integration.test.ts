...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("octopus-daemon-install-int-");
    configPath = path.join(tempHome, "octopus.json");
    process.env.HOME = tempHome;
    process.env.OCTOPUS_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.OCTOPUS_GATEWAY_TOKEN).toBeUndefined();
  });
});
