...
const SANDBOX_EXAMPLES = {
  main: [
    ["octopus sandbox list", "List all sandbox containers."],
    ["octopus sandbox list --browser", "List only browser containers."],
    ["octopus sandbox recreate --all", "Recreate all containers."],
    ["octopus sandbox recreate --session main", "Recreate a specific session."],
    ["octopus sandbox recreate --agent mybot", "Recreate agent containers."],
    ["octopus sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["octopus sandbox list", "List all sandbox containers."],
    ["octopus sandbox list --browser", "List only browser containers."],
    ["octopus sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["octopus sandbox recreate --all", "Recreate all containers."],
    ["octopus sandbox recreate --session main", "Recreate a specific session."],
    ["octopus sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["octopus sandbox recreate --browser --all", "Recreate only browser containers."],
    ["octopus sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["octopus sandbox explain", "Show effective sandbox config."],
    ["octopus sandbox explain --session agent:main:main", "Explain a specific session."],
    ["octopus sandbox explain --agent work", "Explain an agent sandbox."],
    ["octopus sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.octopus.ai/cli/sandbox")}\n`,
    )
    .action(() => {
