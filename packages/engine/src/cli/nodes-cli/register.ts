    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["octopus nodes status", "List known nodes with live status."],
          ["octopus nodes pairing pending", "Show pending node pairing requests."],
          ['octopus nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["octopus nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.octopus.ai/cli/nodes")}\n`,
    );
