...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      OCTOPUS_GATEWAY_PORT: String(params.port),
      ...(params.token ? { OCTOPUS_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OCTOPUS_STATE_DIR = "/tmp/octopus-cli-state";
    process.env.OCTOPUS_CONFIG_PATH = "/tmp/octopus-cli-state/octopus.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OCTOPUS_PROFILE: "dev",
        OCTOPUS_STATE_DIR: "/tmp/octopus-daemon-state",
        OCTOPUS_CONFIG_PATH: "/tmp/octopus-daemon-state/octopus.json",
        OCTOPUS_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.octopus.gateway.plist",
    });
