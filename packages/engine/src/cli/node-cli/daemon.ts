...
function renderNodeServiceStartHints(): string[] {
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("octopus node install"),
    startCommand: formatCliCommand("octopus node start"),
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("octopus node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const [loaded, command, runtime] = await Promise.all([
    ...
    service
      ...
      .catch((err: any): GatewayServiceRuntime => ({ status: "unknown", detail: String(err) })),
  ]);
  ...
  const hintEnv = {
    ...baseEnv,
    OCTOPUS_LOG_PREFIX: baseEnv.OCTOPUS_LOG_PREFIX ?? "node",
