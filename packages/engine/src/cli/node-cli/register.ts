    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "octopus node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["octopus node status", "Check node host service status."],
          ["octopus node install", "Install the node host service."],
          ["octopus node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.octopus.ai/cli/node")}\n`,
    );
