...
function baseCfg(): OctopusConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
    ...
    messages: {
      groupChat: { mentionPatterns: ["@octopus"] },
    },
...
function buildDispatchContextPayload(params: { cfg: OctopusConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@octopus hello",
      bodyText: "@octopus hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
