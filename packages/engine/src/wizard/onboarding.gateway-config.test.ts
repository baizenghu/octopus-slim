...
describe("configureGatewayForOnboarding", () => {
  ...
  it("prefers OCTOPUS_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.OCTOPUS_GATEWAY_TOKEN;
    process.env.OCTOPUS_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.OCTOPUS_GATEWAY_PASSWORD;
    process.env.OCTOPUS_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "OCTOPUS_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.OCTOPUS_GATEWAY_PASSWORD = previous;
      }
    }
  });

  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.OCTOPUS_GATEWAY_TOKEN;
    process.env.OCTOPUS_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "OCTOPUS_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.OCTOPUS_GATEWAY_TOKEN = previous;
      }
    }
  });
