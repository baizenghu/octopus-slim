...
// SLIM: tui/ removed — import { runTui } from "../tui/tui.js";
...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.octopus.ai/gateway/health",
          "https://docs.octopus.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.octopus.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.octopus/octopus.json (gateway.auth.token) or OCTOPUS_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("octopus config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("octopus doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("octopus dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      // SLIM: tui/ removed — runTui unavailable in slim build
      launchedTui = true;
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Octopus."
            : "Copy/paste this URL in a browser on this machine to control Octopus.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("octopus dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.octopus.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.octopus.ai/security",
    "Security",
  );
    ...
    if (webSearchEnabled !== false && hasKey) {
      await prompter.note(
        [
          ...
          `Provider: ${label}`,
          ...(keySource ? [keySource] : []),
          "Docs: https://docs.octopus.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
    } else if (!hasKey) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("octopus configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.octopus.ai/tools/web"}`,
          "Docs: https://docs.octopus.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("octopus configure --section web")}`,
          "",
          "Docs: https://docs.octopus.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://octopus.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Octopus."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Octopus.",
  );
