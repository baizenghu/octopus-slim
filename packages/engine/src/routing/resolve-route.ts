...
function listAgents(cfg: OctopusConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OctopusConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OctopusConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OctopusConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: OctopusConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OctopusConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<OctopusConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OctopusConfig,
  {
    bindingsRef: OctopusConfig["bindings"];
    agentsRef: OctopusConfig["agents"];
    sessionRef: OctopusConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OctopusConfig): Map<string, ResolvedAgentRoute> {
