...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: OctopusConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: OctopusConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/octopus-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<OctopusConfig["bindings"]>[number];
