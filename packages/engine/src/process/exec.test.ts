...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        OCTOPUS_BASE_ENV: "base",
        OCTOPUS_TO_REMOVE: undefined,
      },
      env: {
        OCTOPUS_TEST_ENV: "ok",
      },
    ...
    expect(resolved.OCTOPUS_BASE_ENV).toBe("base");
    expect(resolved.OCTOPUS_TEST_ENV).toBe("ok");
    expect(resolved.OCTOPUS_TO_REMOVE).toBeUndefined();
    expect(resolved.OCTOPUS_CLI).toBe(OCTOPUS_CLI_ENV_VALUE);
  });
