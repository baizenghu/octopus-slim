  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.OCTOPUS_HOME;
        delete env.OCTOPUS_STATE_DIR;
        delete env.OCTOPUS_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "octopus.mjs");
