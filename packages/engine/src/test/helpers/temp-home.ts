...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OCTOPUS_HOME.
  delete process.env.OCTOPUS_HOME;
  process.env.OCTOPUS_STATE_DIR = path.join(base, ".octopus");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "octopus-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".octopus", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
