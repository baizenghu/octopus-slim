...
// Prefix for Octopus-generated action IDs to scope our handler
const OCTOPUS_ACTION_PREFIX = "octopus:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${OCTOPUS_ACTION_PREFIX}`);

  // Handle Octopus modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
