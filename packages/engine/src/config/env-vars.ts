...
function collectConfigEnvVarsByTarget(cfg?: OctopusConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: OctopusConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: OctopusConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: OctopusConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.OCTOPUS_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
