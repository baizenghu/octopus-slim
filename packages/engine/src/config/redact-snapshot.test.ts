...
import type { ConfigFileSnapshot } from "./types.octopus.js";
import { OctopusSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(OctopusSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.octopus/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/octopus/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/octopus/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/octopus/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/octopus/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OctopusSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OctopusConfig";
    const hints = mainSchemaHints;
