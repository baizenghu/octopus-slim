...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("octopus-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "octopus-sandbox-browser-global" },
      agentBrowser: { network: "octopus-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("octopus-sandbox-browser-agent");
  });
