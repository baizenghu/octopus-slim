...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "octopus.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    OCTOPUS_PLUGIN_MANIFEST_CACHE_MS: process.env.OCTOPUS_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-config-plugin-validation-"));
    ...
    process.env.OCTOPUS_STATE_DIR = path.join(suiteHome, ".octopus");
    process.env.OCTOPUS_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.OCTOPUS_STATE_DIR === undefined) {
      delete process.env.OCTOPUS_STATE_DIR;
    } else {
      process.env.OCTOPUS_STATE_DIR = envSnapshot.OCTOPUS_STATE_DIR;
    }
    if (envSnapshot.OCTOPUS_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.OCTOPUS_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.OCTOPUS_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.OCTOPUS_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
