...
import type { OctopusConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OctopusConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: OctopusConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OctopusConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OctopusSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OctopusConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OctopusConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as OctopusConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as OctopusConfig,
  };
}
