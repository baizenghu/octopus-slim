...
import type { OctopusConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOctopusVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  octopus config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  octopus config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: OctopusConfig,
  pathSegments: string[],
): { changed: boolean; next: OctopusConfig } {
...
function coerceConfig(value: unknown): OctopusConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OctopusConfig;
}
...
function stampConfigVersion(cfg: OctopusConfig): OctopusConfig {
...
function warnIfConfigFromFuture(cfg: OctopusConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOctopusVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Octopus (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OctopusConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OctopusConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OctopusConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OctopusConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OctopusConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OCTOPUS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OCTOPUS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OCTOPUS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OCTOPUS_WATCH_SESSION === "string" &&
        deps.env.OCTOPUS_WATCH_SESSION.trim().length > 0
          ? deps.env.OCTOPUS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OCTOPUS_WATCH_COMMAND === "string" &&
        deps.env.OCTOPUS_WATCH_COMMAND.trim().length > 0
          ? deps.env.OCTOPUS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OCTOPUS_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: OctopusConfig | null = null;
let runtimeConfigSourceSnapshot: OctopusConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OCTOPUS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OCTOPUS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): OctopusConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): OctopusConfig | null {
...
export function loadConfig(): OctopusConfig {
...
export async function readBestEffortConfig(): Promise<OctopusConfig> {
...
export async function writeConfigFile(
  ...
  // edits to octopus.json remain visible (no stale snapshot).
}
