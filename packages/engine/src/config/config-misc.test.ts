...
import { buildWebSearchProviderConfig, withTempHome, writeOctopusConfig } from "./test-helpers.js";
import { OctopusSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = OctopusSchema.safeParse({
      $schema: "https://octopus.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://octopus.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = OctopusSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = OctopusSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeOctopusConfig(home, {
        gateway: { bind: "${OCTOPUS_BIND}" },
      });

      const prev = process.env.OCTOPUS_BIND;
      process.env.OCTOPUS_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.OCTOPUS_BIND;
        } else {
          process.env.OCTOPUS_BIND = prev;
        }
      }
    });
  });
