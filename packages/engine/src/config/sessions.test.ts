...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OCTOPUS_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-octopus",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-octopus#general");
  });
  ...
  it("derives session transcripts dir from OCTOPUS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OCTOPUS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when OCTOPUS_STATE_DIR differs from stored paths", () => {
