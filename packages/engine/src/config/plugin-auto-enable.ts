...
function isWhatsAppConfigured(cfg: OctopusConfig): boolean {
...
function isGenericChannelConfigured(cfg: OctopusConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: OctopusConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OctopusConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OctopusConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: OctopusConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OctopusConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OctopusConfig, pluginId: string): OctopusConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
