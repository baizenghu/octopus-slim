...
describe("resolveInitialTuiAgentId", () => {
  const cfg: OctopusConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/octopus" },
        { id: "ops", workspace: "/tmp/octopus/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/octopus/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run octopus devices list");
    expect(state.pairingHint).toContain("octopus devices list");
  });
