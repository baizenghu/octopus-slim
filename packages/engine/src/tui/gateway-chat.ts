...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set OCTOPUS_GATEWAY_TOKEN/OCTOPUS_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.OCTOPUS_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.OCTOPUS_GATEWAY_PASSWORD);
