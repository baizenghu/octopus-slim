...
const TEMP_MEDIA_PREFIX = "octopus-media-";
...
function createGroqAudioConfig(): OctopusConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OctopusConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OctopusConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OctopusConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredOctopusTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        OCTOPUS_AGENT_DIR: isolatedAgentDir,
