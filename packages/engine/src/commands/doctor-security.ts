...
import type { OctopusConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: OctopusConfig): string[] {
...
export async function noteSecurityWarnings(cfg: OctopusConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("octopus security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.octopus/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("octopus approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.octopus.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("octopus configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("octopus config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("octopus doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "octopus config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("octopus config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('octopus config set session.dmScope "per-channel-peer"') +
