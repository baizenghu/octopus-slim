...
import { readConfigFileSnapshot, writeConfigFile, type OctopusConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("octopus config set gateway.auth.mode token")} or ${formatCliCommand("octopus config set gateway.auth.mode password")}.`,
  ].join(" ");
}
