...
describe("readServiceStatusSummary", () => {
  it("marks Octopus-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["octopus", "gateway", "run"] })),
    ...
    expect(summary.managedByOctopus).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByOctopus).toBe(false);
