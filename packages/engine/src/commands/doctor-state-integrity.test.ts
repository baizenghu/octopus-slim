...
type EnvSnapshot = {
  HOME?: string;
  OCTOPUS_HOME?: string;
  OCTOPUS_STATE_DIR?: string;
  OCTOPUS_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OCTOPUS_HOME: process.env.OCTOPUS_HOME,
    OCTOPUS_STATE_DIR: process.env.OCTOPUS_STATE_DIR,
    OCTOPUS_OAUTH_DIR: process.env.OCTOPUS_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OctopusConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OctopusConfig) {
...
async function runStateIntegrityText(cfg: OctopusConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "octopus-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.OCTOPUS_HOME = tempHome;
    process.env.OCTOPUS_STATE_DIR = path.join(tempHome, ".octopus");
    delete process.env.OCTOPUS_OAUTH_DIR;
    fs.mkdirSync(process.env.OCTOPUS_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OCTOPUS_OAUTH_DIR is explicitly configured", async () => {
    process.env.OCTOPUS_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OctopusConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints octopus-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OctopusConfig = {};
    ...
    expect(text).toMatch(/octopus sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/octopus sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /octopus sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
