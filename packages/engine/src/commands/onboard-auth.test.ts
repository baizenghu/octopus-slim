...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OCTOPUS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("octopus-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-oauth-sync-"));
    ...
    process.env.OCTOPUS_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-oauth-nosync-"));
    process.env.OCTOPUS_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OCTOPUS_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-oauth-external-"));
    process.env.OCTOPUS_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OCTOPUS_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OCTOPUS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("octopus-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
