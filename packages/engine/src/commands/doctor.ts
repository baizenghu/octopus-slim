...
import { resolveOctopusPackageRoot } from "../infra/octopus-root.js";
...
function resolveMode(cfg: OctopusConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Octopus doctor");

  const root = await resolveOctopusPackageRoot({
  ...
  let cfg: OctopusConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("octopus configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("octopus config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("octopus setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("octopus config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("octopus config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("octopus doctor --fix")}" to apply changes.`);
  }
