...
beforeAll(() => {
  envSnapshot = captureEnv(["OCTOPUS_PROFILE"]);
  process.env.OCTOPUS_PROFILE = "isolated";
});
...
vi.mock("../infra/octopus-root.js", () => ({
  resolveOctopusPackageRoot: vi.fn().mockResolvedValue("/tmp/octopus"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/octopus/pnpm-lock.yaml",
      markerPath: "/tmp/octopus/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.octopus.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.octopus.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("octopus status --all") ||
          line.includes("octopus --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("OCTOPUS_GATEWAY_TOKEN", "abcd1234", async () => {
