...
export function normalizeCompatibilityConfigValues(cfg: OctopusConfig): {
  config: OctopusConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: OctopusConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as OctopusConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as OctopusConfig["browser"],
    };
    changes.push(
