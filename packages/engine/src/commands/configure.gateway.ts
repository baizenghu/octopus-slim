...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "OCTOPUS_GATEWAY_TOKEN",
          placeholder: "OCTOPUS_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like OCTOPUS_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Octopus will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Octopus trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.octopus.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
