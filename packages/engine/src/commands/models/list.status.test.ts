...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOctopusAgentDir: vi.fn().mockReturnValue("/tmp/octopus-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/octopus-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/octopus-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/octopus-dev/octopus.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOctopusAgentDir: mocks.resolveOctopusAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/octopus-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOctopusAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/octopus-dev/octopus.json");
    expect(payload.auth.storePath).toBe("/tmp/octopus-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/octopus-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/octopus-agent-custom");
