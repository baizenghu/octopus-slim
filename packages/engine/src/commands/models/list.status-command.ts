...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOctopusAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("octopus models auth setup-token")}\` or \`${formatCliCommand("octopus configure")}\`.`
          : `Run \`${formatCliCommand("octopus configure")}\` or set an API key env var.`;
