...
export async function loadValidConfigOrThrow(): Promise<OctopusConfig> {
...
export async function updateConfig(
  mutator: (cfg: OctopusConfig) => OctopusConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OctopusConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OctopusConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("octopus agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
