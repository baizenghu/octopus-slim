...
function resolveAgentName(cfg: OctopusConfig, agentId: string) {
...
function resolveAgentModel(cfg: OctopusConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OctopusConfig): AgentSummary[] {
