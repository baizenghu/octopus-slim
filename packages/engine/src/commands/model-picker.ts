...
type PromptDefaultModelResult = { model?: string; config?: OctopusConfig };
...
function resolveConfiguredModelRaw(cfg: OctopusConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OctopusConfig): string[] {
...
export function applyPrimaryModel(cfg: OctopusConfig, model: string): OctopusConfig {
...
export function applyModelAllowlist(cfg: OctopusConfig, models: string[]): OctopusConfig {
  const defaults = cfg.agents?.defaults;
