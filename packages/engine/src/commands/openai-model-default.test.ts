...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OctopusConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OctopusConfig) => { changed: boolean; next: OctopusConfig };
  defaultModel: string;
  overrideConfig: OctopusConfig;
  alreadyDefaultConfig: OctopusConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OctopusConfig) => config,
      applyDefaultConfig: (config: OctopusConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OctopusConfig = { agents: { defaults: {} } };
