...
function rawKeyValue(config: OctopusConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: OctopusConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: OctopusConfig, provider: SearchProvider): OctopusConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: OctopusConfig, result: OctopusConfig): OctopusConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with octopus configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Octopus will store a reference instead of the API key.",
