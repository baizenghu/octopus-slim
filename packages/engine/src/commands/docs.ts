...
const SEARCH_TOOL = "https://docs.octopus.ai/mcp.SearchOctopus";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.octopus.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('octopus docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.octopus.ai/");
      runtime.log(`Search: ${formatCliCommand('octopus docs "your query"')}`);
    }
    return;
  }
