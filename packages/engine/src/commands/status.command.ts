...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByOctopus ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByOctopus ? "installed · " : "";
  ...
  runtime.log(theme.heading("Octopus status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`octopus devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("octopus devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("octopus devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("octopus security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("octopus security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.octopus.ai/faq");
  runtime.log("Troubleshooting: https://docs.octopus.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("octopus status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("octopus logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("octopus status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("octopus gateway probe")}`);
  }
}
