...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("falls back to OCTOPUS_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
      ...
      {
        OCTOPUS_GATEWAY_TOKEN: "env-fallback-token",
      } as NodeJS.ProcessEnv,
    );
  ...
  it("falls back to OCTOPUS_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
    ...
    const required = shouldRequireGatewayTokenForInstall(
      {
        ...
        env: {
          vars: {
            OCTOPUS_GATEWAY_PASSWORD: "configured-password", // pragma: allowlist secret
          },
        },
      } as OctopusConfig,
      {} as NodeJS.ProcessEnv,
