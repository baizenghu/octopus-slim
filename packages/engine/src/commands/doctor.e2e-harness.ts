...
export const resolveOctopusPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOctopusPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveOctopusPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OCTOPUS_STATE_DIR;
  originalUpdateInProgress = process.env.OCTOPUS_UPDATE_IN_PROGRESS;
  process.env.OCTOPUS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "octopus-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OCTOPUS_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OCTOPUS_UPDATE_IN_PROGRESS;
  } else {
    process.env.OCTOPUS_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
