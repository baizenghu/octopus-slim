...
import { OctopusSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: OctopusConfig): {
  config: OctopusConfig;
  removed: string[];
} {
  const parsed = OctopusSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: OctopusConfig): void {
