...
const ensureOctopusModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveOctopusAgentDir = vi.fn().mockReturnValue("/tmp/octopus-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureOctopusModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveOctopusAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureOctopusModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureOctopusModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureOctopusModelsJson).toHaveBeenCalled();
    expect(ensureOctopusModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
