...
function prependInternalEventContext(
  ...
  if (body.includes("Octopus runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("octopus agents list")}" to see configured agents.`,
      );
    }
  }
...
async function agentCommandInternal(
  ...
  try {
    ...
    const skillFilterChanged = !matchesSkillFilter(sessionEntry?.skillsSnapshot?.skillFilter, skillFilter);
    const needsSkillsSnapshot = isNewSession || !sessionEntry?.skillsSnapshot || skillFilterChanged;
