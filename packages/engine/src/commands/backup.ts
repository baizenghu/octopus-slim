...
export async function backupCreateCommand(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Octopus config file was found to back up."
        : "No local Octopus state was found to back up.",
    );
  }
  ...
  if (!opts.dryRun) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-backup-"));
