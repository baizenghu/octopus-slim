...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: OctopusConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: OctopusConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: OctopusConfig) => string[];
  isConfigured?: (account: unknown, cfg: OctopusConfig) => boolean;
  isEnabled?: (account: unknown, cfg: OctopusConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ OCTOPUS_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        octopus: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.OCTOPUS_GATEWAY_TOKEN;
    const prevPassword = process.env.OCTOPUS_GATEWAY_PASSWORD;
    delete process.env.OCTOPUS_GATEWAY_TOKEN;
    delete process.env.OCTOPUS_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.OCTOPUS_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.OCTOPUS_GATEWAY_PASSWORD;
      } else {
        process.env.OCTOPUS_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/octopus-safe-bins"];
    const cfg: OctopusConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("octopus-sbx-browser-old\noctopus-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "octopus-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "octopus-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("octopus-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("octopus-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "octopus-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "octopus-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-octopus.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: OctopusConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: OctopusConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): OctopusConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: OctopusConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: OctopusConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: OctopusConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.octopus/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: OctopusConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.OCTOPUS_GATEWAY_TOKEN;
    process.env.OCTOPUS_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: OctopusConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<OctopusConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: OctopusConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.octopus",
      configPath: "/Users/test/Dropbox/.octopus/octopus.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: OctopusConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "octopus.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "octopus.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@octopus/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@octopus/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.OCTOPUS_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.OCTOPUS_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
