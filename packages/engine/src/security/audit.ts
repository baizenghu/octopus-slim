...
import type { ConfigFileSnapshot, OctopusConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: OctopusConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Octopus state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Octopus state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.OCTOPUS_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("octopus security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: OctopusConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: OctopusConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: OctopusConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("octopus status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("octopus security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set OCTOPUS_GATEWAY_TOKEN/OCTOPUS_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("octopus security audit --deep")}".`,
    });
  }
