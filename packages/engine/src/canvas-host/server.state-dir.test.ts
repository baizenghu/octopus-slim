...
describe("canvas host state dir defaults", () => {
  it("uses OCTOPUS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("octopus-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Octopus Canvas");
      } finally {
        await handler.close();
      }
    });
