...
function resolveVoiceTtsConfig(params: { cfg: OctopusConfig; override?: TtsConfig }): {
  cfg: OctopusConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOctopusTmpDir(), "discord-voice-"));
