...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OCTOPUS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*octopus-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "octopus";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "octopus");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "octopus-gw");
  ...
  records.push(`_octopus-gw._tcp IN PTR ${instanceLabel}._octopus-gw._tcp`);
  records.push(`${instanceLabel}._octopus-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._octopus-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; octopus-content-hash: ${contentHash}\n${contentBody}`;
}
