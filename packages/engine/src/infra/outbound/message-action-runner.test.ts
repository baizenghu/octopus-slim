...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred Octopus tmp root", async () => {
    const tmpRoot = resolvePreferredOctopusTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideOctopus = path.join(os.tmpdir(), "outside-octopus", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
