...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OctopusConfig | null = null;
  ...
  get(key: string, cfg: OctopusConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OctopusConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OctopusConfig): void {
