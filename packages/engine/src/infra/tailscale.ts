...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.OCTOPUS_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Octopus. You can keep running the web gateway without it: \`${formatCliCommand("octopus gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
