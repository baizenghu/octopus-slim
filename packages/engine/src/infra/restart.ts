...
export function triggerOctopusRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.OCTOPUS_SYSTEMD_UNIT,
      process.env.OCTOPUS_PROFILE,
    );
  ...
  const label =
    process.env.OCTOPUS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OCTOPUS_PROFILE);
