...
import { POSIX_OCTOPUS_TMP_DIR, resolvePreferredOctopusTmpDir } from "./tmp-octopus-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOctopusTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `octopus-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredOctopusTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_OCTOPUS_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOctopusTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredOctopusTmpDir", () => {
  it("prefers /tmp/octopus when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OCTOPUS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/octopus when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OCTOPUS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/octopus when /tmp/octopus is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/octopus when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/octopus is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/octopus is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/octopus is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Octopus temp dir/);
  });
