 ...
 * returning control to the caller (typically `triggerOctopusRestart` →
...
/**
 * Parse octopus gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("octopus")) {
 ...
 * Returns only PIDs that belong to octopus gateway processes (not the current process).
 */
