...
const DEFAULT_PACKAGE_NAME = "octopus";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["octopus", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "octopus.mjs");
    ...
    const doctorStep = await runStep(
      step("octopus doctor", doctorArgv, gitRoot, { OCTOPUS_UPDATE_IN_PROGRESS: "1" }),
    );
