...
function candidateBinDirs(opts: EnsureOctopusPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `octopus` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "octopus");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OCTOPUS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "octopus"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `octopus` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOctopusCliOnPath(opts: EnsureOctopusPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OCTOPUS_PATH_BOOTSTRAPPED)) {
    return;
  }
