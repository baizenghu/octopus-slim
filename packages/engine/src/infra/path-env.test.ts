...
let ensureOctopusCliOnPath: typeof import("./path-env.js").ensureOctopusCliOnPath;

describe("ensureOctopusCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OCTOPUS_PATH_BOOTSTRAPPED",
    "OCTOPUS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOctopusCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling octopus exists", () => {
    const tmp = abs("/tmp/octopus-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "octopus");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OCTOPUS_PATH_BOOTSTRAPPED;

    ensureOctopusCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.OCTOPUS_PATH_BOOTSTRAPPED = "1";
    ensureOctopusCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/octopus-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "octopus");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/octopus-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "octopus");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "octopus");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/octopus-path/case-linuxbrew");
