...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __octopusDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__octopusDiagnosticEventsState) {
    globalStore.__octopusDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__octopusDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OctopusConfig): boolean {
