...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OCTOPUS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OCTOPUS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set OCTOPUS_APNS_TEAM_ID and OCTOPUS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.OCTOPUS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OCTOPUS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OCTOPUS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OCTOPUS_APNS_PRIVATE_KEY_P8 or OCTOPUS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OCTOPUS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createOctopusPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    octopus: createOctopusPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
