...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OCTOPUS_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OCTOPUS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOctopusPackageRoot } = await import("./octopus-root.js"));
    ...
    vi.mocked(resolveOctopusPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveOctopusPackageRoot).mockResolvedValue("/opt/octopus");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/octopus/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
