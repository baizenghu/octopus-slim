...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__octopus_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "octopus-root");
...
describe("resolveOctopusPackageRoot", () => {
  let resolveOctopusPackageRoot: typeof import("./octopus-root.js").resolveOctopusPackageRoot;
  let resolveOctopusPackageRootSync: typeof import("./octopus-root.js").resolveOctopusPackageRootSync;

  beforeAll(async () => {
    ({ resolveOctopusPackageRoot, resolveOctopusPackageRootSync } =
      await import("./octopus-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "octopus");
    const pkgRoot = path.join(project, "node_modules", "octopus");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "octopus" }));

    expect(resolveOctopusPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "octopus");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "octopus.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "octopus" }));

    expect(resolveOctopusPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOctopusPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveOctopusPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveOctopusPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-octopus package roots", async () => {
    const pkgRoot = fx("not-octopus");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-octopus" }));

    expect(resolveOctopusPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOctopusPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOctopusPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
