...
describe("resolveEffectiveHomeDir", () => {
  it("prefers OCTOPUS_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands OCTOPUS_HOME when set to ~", () => {
    const env = {
      OCTOPUS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for OCTOPUS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { OCTOPUS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when OCTOPUS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ OCTOPUS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { OCTOPUS_HOME: "/srv/octopus-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/octopus-home")}/x`);
  });
