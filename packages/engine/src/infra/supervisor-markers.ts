const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "OCTOPUS_LAUNCHD_LABEL"],
  systemd: ["OCTOPUS_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["OCTOPUS_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "OCTOPUS_SERVICE_MARKER",
  "OCTOPUS_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.OCTOPUS_SERVICE_MARKER?.trim();
    const serviceKind = env.OCTOPUS_SERVICE_KIND?.trim();
