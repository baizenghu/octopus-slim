...
import { triggerOctopusRestart } from "./restart.js";
 ...
 * - OCTOPUS_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.OCTOPUS_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    // See: https://github.com/octopus/octopus/issues/39760
    if (supervisor === "schtasks") {
      const restart = triggerOctopusRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
