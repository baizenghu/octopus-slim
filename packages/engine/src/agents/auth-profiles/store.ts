 ...
 * A common mistake — caused by the similarity with the `octopus.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (octopus.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.OCTOPUS_AUTH_STORE_READONLY === "1";
