...
const TOOL_DENY_FOR_XAI_PROVIDERS = new Set<string>();
...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending Octopus's
...
function resolveExecConfig(params: { cfg?: OctopusConfig; agentId?: string }) {
...
export function createOctopusCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOctopusReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
