...
const GOOGLE_LIVE_SINGLE_KEY = "OCTOPUS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OCTOPUS_LIVE_ANTHROPIC_KEY",
    listVar: "OCTOPUS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OCTOPUS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OCTOPUS_LIVE_${base}_KEY`;
