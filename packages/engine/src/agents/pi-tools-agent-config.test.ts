...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OctopusConfig) {
    return createOctopusCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<OctopusConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<OctopusConfig["agents"]>["list"]>[number]["tools"];
  }): OctopusConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createOctopusCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createOctopusCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createOctopusCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createOctopusCodingTools({
      config: cfg,
    ...
    const bobTools = createOctopusCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createOctopusCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createOctopusCodingTools({
      config: cfg,
