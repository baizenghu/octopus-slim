...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    octopus: process.env.OCTOPUS_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.octopus === undefined) {
      delete process.env.OCTOPUS_GATEWAY_TOKEN;
    } else {
      process.env.OCTOPUS_GATEWAY_TOKEN = envSnapshot.octopus;
    }
  ...
  it("uses OCTOPUS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OCTOPUS_GATEWAY_TOKEN = "local-env-token";
