...
type WebSearchConfig = NonNullable<OctopusConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OctopusConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "brave") {
    return {
      ...
      message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("octopus configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
  ...
  return withTrustedWebSearchEndpoint(
    {
      ...
      init: {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://octopus.ai",
          "X-Title": "Octopus Web Search",
        },
