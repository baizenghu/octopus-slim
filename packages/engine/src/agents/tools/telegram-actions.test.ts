...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OctopusConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OctopusConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OctopusConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OctopusConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: OctopusConfig, accountId = "media") {
    await handleTelegramAction(
