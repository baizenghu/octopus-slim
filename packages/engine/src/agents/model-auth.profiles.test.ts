...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-oauth-"));

    try {
      const agentDir = path.join(tempDir, "agent");
      await withEnvAsync(
        {
          OCTOPUS_STATE_DIR: tempDir,
          OCTOPUS_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.OCTOPUS_AGENT_DIR, {
