...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const octopusTmpDir = path.resolve(resolvePreferredOctopusTmpDir());
  if (!isPathInside(octopusTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: octopusTmpDir });
  return resolved;
}
