...
function resolveAttachmentLimits(config: OctopusConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".octopus", "attachments");
  const relDir = path.posix.join(".octopus", "attachments", attachmentId);
