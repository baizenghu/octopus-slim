...
function resolveSkillsLimits(config?: OctopusConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "octopus-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "octopus-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveOctopusMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: OctopusConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`octopus skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
