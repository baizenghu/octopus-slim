...
import {
  applyOctopusManifestInstallCommonFields,
  getFrontmatterString,
  normalizeStringList,
  parseOctopusManifestInstallBase,
  parseFrontmatterBool,
  resolveOctopusManifestBlock,
  resolveOctopusManifestInstall,
  resolveOctopusManifestOs,
  resolveOctopusManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  OctopusSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseOctopusManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyOctopusManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveOctopusMetadata(
  frontmatter: ParsedSkillFrontmatter,
): OctopusSkillMetadata | undefined {
  const metadataObj = resolveOctopusManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveOctopusManifestRequires(metadataObj);
  const install = resolveOctopusManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveOctopusManifestOs(metadataObj);
  return {
