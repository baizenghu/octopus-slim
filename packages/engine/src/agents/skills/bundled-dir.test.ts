...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["OCTOPUS_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns OCTOPUS_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-bundled-override-"));
    process.env.OCTOPUS_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "octopus" }));
