    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "octopus.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "octopus.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "octopus.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("octopus-");
  const acpxRoot = await tempDirs.make("octopus-acpx-plugin-");
  const helperRoot = await tempDirs.make("octopus-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("octopus-");
  const pluginRoot = await tempDirs.make("octopus-plugin-");
  const outsideDir = await tempDirs.make("octopus-outside-");
