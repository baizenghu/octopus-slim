...
function buildCoreToolGroupMap() {
  ...
  const octopusTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOctopusGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:octopus": octopusTools,
