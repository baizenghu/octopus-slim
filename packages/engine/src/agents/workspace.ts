...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OCTOPUS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".octopus", `workspace-${profile}`);
  }
  return path.join(home, ".octopus", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".octopus";
