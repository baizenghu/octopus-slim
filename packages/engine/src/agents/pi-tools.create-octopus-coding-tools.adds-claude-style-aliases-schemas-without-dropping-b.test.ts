...
const defaultTools = createOctopusCodingTools({ senderIsOwner: true });
...
describe("createOctopusCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: OctopusConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createOctopusCodingTools({
    ...
    const anthropicTools = createOctopusCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: OctopusConfig = {
      tools: {
        exec: {
    ...
    const allowed = createOctopusCodingTools({
    ...
    const denied = createOctopusCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createOctopusCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
