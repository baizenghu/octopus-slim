...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: OctopusConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/octopus" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OctopusConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OctopusConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OctopusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: OctopusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: OctopusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OctopusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: OctopusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/octopus");
  });

  it("uses OCTOPUS_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "octopus-home");
    vi.stubEnv("OCTOPUS_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OctopusConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".octopus", "workspace"));
  });

  it("uses OCTOPUS_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "octopus-home");
    vi.stubEnv("OCTOPUS_HOME", home);
    // Clear state dir so it falls back to OCTOPUS_HOME
    vi.stubEnv("OCTOPUS_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OctopusConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".octopus", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/octopus-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/octopus-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "octopus-agent-scope-"));
