...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["OCTOPUS_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-subagent-"));
  ...
  it("uses isolated temp state when OCTOPUS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "octopus-test-state"));
  });
});
