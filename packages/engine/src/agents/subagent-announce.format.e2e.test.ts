...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/octopus/octopus/issues/31298
    previousFastTestEnv = process.env.OCTOPUS_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.OCTOPUS_TEST_FAST;
      return;
    }
    process.env.OCTOPUS_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // OCTOPUS_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Octopus runtime context (internal):");
