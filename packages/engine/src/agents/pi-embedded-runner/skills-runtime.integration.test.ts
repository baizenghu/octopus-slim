...
const originalBundledDir = process.env.OCTOPUS_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("octopus-bundled-");
  const workspaceDir = await createTempDir("octopus-workspace-");
...
afterEach(async () => {
  process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
