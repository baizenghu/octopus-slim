...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    workspaceRoot: "/tmp/octopus-sandboxes",
    docker: {
    ...
    browser: {
      enabled: true,
      image: "octopus-sandbox-browser:bookworm-slim",
      containerPrefix: "octopus-sbx-browser-",
      network: "octopus-sandbox-browser",
      cdpPort: 9222,
      vncPort: 5900,
      noVncPort: 6080,
      headless: false,
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("OCTOPUS_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("OCTOPUS_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^OCTOPUS_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("does not inject noVNC password env when noVNC is disabled", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries.some((entry) => entry.startsWith("OCTOPUS_BROWSER_NOVNC_PASSWORD="))).toBe(
      false,
    );
