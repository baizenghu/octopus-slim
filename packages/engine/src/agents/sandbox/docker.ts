...
import { markOctopusExecEnv } from "../../infra/octopus-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "octopus.sandbox=1");
  args.push("--label", `octopus.sessionKey=${params.scopeKey}`);
  args.push("--label", `octopus.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `octopus.configHash=${params.configHash}`);
  }
  ...
  const envSanitization = sanitizeEnvVars(markOctopusExecEnv(params.cfg.env ?? {}));
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "octopus.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`octopus sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`octopus sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("octopus sandbox recreate --all");
}
