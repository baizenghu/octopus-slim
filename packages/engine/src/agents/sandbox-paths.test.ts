...
import { resolvePreferredOctopusTmpDir } from "../infra/tmp-octopus-dir.js";
...
async function withOutsideHardlinkInOctopusTmp<T>(
  params: {
    octopusTmpDir: string;
  ...
  const hardlinkPath = path.join(params.octopusTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.octopusTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.octopusTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.octopusTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const octopusTmpDir = resolvePreferredOctopusTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Octopus tmp root",
      media: path.join(octopusTmpDir, "image.png"),
      expected: path.join(octopusTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Octopus tmp root",
      media: pathToFileURL(path.join(octopusTmpDir, "photo.png")).href,
      expected: path.join(octopusTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Octopus tmp root",
      media: path.join(octopusTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(octopusTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(octopusTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside octopus tmp root",
      media: path.join(os.tmpdir(), "outside-octopus", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Octopus tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(octopusTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(octopusTmpDir, { recursive: true });
      const symlinkPath = path.join(octopusTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Octopus tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOctopusTmp(
      {
        octopusTmpDir,
  ...
  it("rejects symlinked Octopus tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOctopusTmp(
