...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OCTOPUS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OCTOPUS_HOME: "/srv/octopus-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/octopus-home"), ".octopus", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".octopus", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("octopus-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-workspace-hardlink-"));
    try {
