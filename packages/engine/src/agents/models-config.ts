...
import { planOctopusModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: OctopusConfig): OctopusConfig {
...
export async function ensureOctopusModelsJson(
  config?: OctopusConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = resolveModelsConfigInput(config);
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveOctopusAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planOctopusModelsJson({
      cfg,
      agentDir,
      env,
