...
test("exec sets OCTOPUS_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-octopus-shell", {
    command: "node -e \"process.stdout.write(process.env.OCTOPUS_SHELL || '')\"",
    pty: true,
  });
