...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("octopus-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("octopus-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("octopus-sbx-");
