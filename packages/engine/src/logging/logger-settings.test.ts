...
let originalOctopusLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.OCTOPUS_TEST_FILE_LOG;
  originalOctopusLogLevel = process.env.OCTOPUS_LOG_LEVEL;
  delete process.env.OCTOPUS_TEST_FILE_LOG;
  delete process.env.OCTOPUS_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.OCTOPUS_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalOctopusLogLevel === undefined) {
    delete process.env.OCTOPUS_LOG_LEVEL;
  } else {
    process.env.OCTOPUS_LOG_LEVEL = originalOctopusLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
