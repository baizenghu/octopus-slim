...
describe("installScheduledTask", () => {
  async function withUserProfileDir(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-schtasks-install-"));
    const env = {
      USERPROFILE: tmpDir,
  ...
  it("writes quoted set assignments and escapes metacharacters", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      expect(schtasksCalls[2]).toEqual(["/Run", "/TN", "Octopus Gateway"]);
    });
  });
    ...
    await withUserProfileDir(async (_tmpDir, env) => {
      const { scriptPath } = await installScheduledTask({
        ...
        environment: {
          ...
          OCTOPUS_GATEWAY_PORT: "18789",
        },
      });

      const script = await fs.readFile(scriptPath, "utf8");
      expect(script).not.toContain('set "PATH=');
      expect(script).toContain('set "OCTOPUS_GATEWAY_PORT=18789"');
    });
  });
});
