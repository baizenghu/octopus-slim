...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    OCTOPUS_PROFILE: profile,
    OCTOPUS_GATEWAY_PORT: String(port),
    OCTOPUS_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OCTOPUS_SYSTEMD_UNIT: systemdUnit,
    OCTOPUS_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    OCTOPUS_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OCTOPUS_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    OCTOPUS_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.OCTOPUS_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OCTOPUS_GATEWAY_TOKEN: gatewayToken,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    OCTOPUS_STATE_DIR: sharedEnv.stateDir,
    OCTOPUS_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.OCTOPUS_STATE_DIR;
  const configPath = env.OCTOPUS_CONFIG_PATH;
