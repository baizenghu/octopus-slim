...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `octopus-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      OCTOPUS_LAUNCHD_LABEL: `ai.octopus.launchd-int-${testId}`,
      OCTOPUS_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
