...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when OCTOPUS_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".octopus", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when OCTOPUS_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", OCTOPUS_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".octopus-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers OCTOPUS_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        OCTOPUS_PROFILE: "rescue",
        OCTOPUS_STATE_DIR: "C:\\State\\octopus",
      },
      expected: path.join("C:\\State\\octopus", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", OCTOPUS_PROFILE: "default" },
      expected: path.join("/home/test", ".octopus", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Octopus Gateway",
          "cd /d C:\\Projects\\octopus",
          "set NODE_ENV=production",
          "set OCTOPUS_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\octopus",
          environment: {
            NODE_ENV: "production",
            OCTOPUS_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\octopus\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\octopus\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Octopus Share\\node.exe" "\\\\fileserver\\Octopus Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Octopus Share\\node.exe",
            "\\\\fileserver\\Octopus Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from OCTOPUS_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ OCTOPUS_STATE_DIR: path.join(tmpDir, "custom-state") }),
