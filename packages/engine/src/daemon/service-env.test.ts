...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OCTOPUS_GATEWAY_PORT).toBe("18789");
    expect(env.OCTOPUS_GATEWAY_TOKEN).toBeUndefined();
    expect(env.OCTOPUS_SERVICE_MARKER).toBe("octopus");
    expect(env.OCTOPUS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OCTOPUS_SERVICE_VERSION).toBe("string");
    expect(env.OCTOPUS_SYSTEMD_UNIT).toBe("octopus-gateway.service");
    expect(env.OCTOPUS_WINDOWS_TASK_NAME).toBe("Octopus Gateway");
    if (process.platform === "darwin") {
      expect(env.OCTOPUS_LAUNCHD_LABEL).toBe("ai.octopus.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OCTOPUS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OCTOPUS_SYSTEMD_UNIT).toBe("octopus-gateway-work.service");
    expect(env.OCTOPUS_WINDOWS_TASK_NAME).toBe("Octopus Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.OCTOPUS_LAUNCHD_LABEL).toBe("ai.octopus.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through OCTOPUS_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", OCTOPUS_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.OCTOPUS_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to OCTOPUS_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.OCTOPUS_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers OCTOPUS_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OCTOPUS_GATEWAY_TOKEN: "octopus-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.OCTOPUS_GATEWAY_TOKEN).toBe("octopus-token");
  });

  it("omits OCTOPUS_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".octopus"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OCTOPUS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".octopus-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OCTOPUS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".octopus"));
  });

  it("uses OCTOPUS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OCTOPUS_STATE_DIR: "/var/lib/octopus" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/octopus"));
  });

  it("expands ~ in OCTOPUS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OCTOPUS_STATE_DIR: "~/octopus-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/octopus-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OCTOPUS_STATE_DIR: "C:\\State\\octopus" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\octopus");
  });
});
