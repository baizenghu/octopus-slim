...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds OCTOPUS_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `octopus gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service OCTOPUS_GATEWAY_TOKEN does not match gateway.auth.token in octopus.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.OCTOPUS_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.OCTOPUS_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `octopus gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
