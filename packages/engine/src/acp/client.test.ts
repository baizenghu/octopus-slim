...
const createTempDir = () => tempDirs.make("octopus-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets OCTOPUS_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "octopus",
    });

    expect(env.OCTOPUS_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("octopus");
  });

  it("overrides pre-existing OCTOPUS_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      OCTOPUS_SHELL: "wrong",
    });
    expect(env.OCTOPUS_SHELL).toBe("acp-client");
  ...
  it("preserves OCTOPUS_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        OCTOPUS_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["OCTOPUS_SHELL", openAiApiKeyEnv]) },
    );
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "octopus", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "octopus", "dist", "entry.js");
    const shimPath = path.join(dir, "octopus.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\octopus\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/octopus-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/octopus-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/octopus-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
