...
import { resolveOctopusPackageRootSync } from "../infra/octopus-root.js";
...
import { discoverOctopusPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveOctopusPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`octopus/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OctopusPluginDefinition;
  register?: OctopusPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OctopusPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OctopusPluginDefinition;
...
export function loadOctopusPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOctopusPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "octopus/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OctopusPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as OctopusPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
