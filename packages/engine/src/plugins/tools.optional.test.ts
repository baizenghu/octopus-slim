...
const loadOctopusPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadOctopusPlugins: (params: unknown) => loadOctopusPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadOctopusPluginsMock.mockReturnValue(registry);
  return registry;
}
...
describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadOctopusPluginsMock.mockClear();
  });
