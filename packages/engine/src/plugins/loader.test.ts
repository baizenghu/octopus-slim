...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "octopus-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.OCTOPUS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "octopus.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadOctopusPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          octopus: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadOctopusPlugins>) {
...
function useNoBundledPlugins() {
  process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadOctopusPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.OCTOPUS_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOctopusPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOctopusPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadOctopusPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadOctopusPlugins(options);
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "octopus.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "octopus.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ OCTOPUS_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("octopus/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadOctopusPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadOctopusPlugins({
        cache: false,
    ...
    execFileSync(process.execPath, ["--import", "tsx", "--input-type=module", "-e", script], {
      ...
      env: {
        ...process.env,
        OCTOPUS_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
      },
      encoding: "utf-8",
      stdio: "pipe",
    });
