...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing octopus.extensions; update the plugin package to include octopus.extensions (for example ["./dist/index.js"]). See https://docs.octopus.ai/help/troubleshooting#plugin-install-fails-with-missing-octopus-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_OCTOPUS_EXTENSIONS: "missing_octopus_extensions",
  EMPTY_OCTOPUS_EXTENSIONS: "empty_octopus_extensions",
...
function ensureOctopusExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_OCTOPUS_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json octopus.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_OCTOPUS_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureOctopusExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from octopus.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-octopus"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "octopus security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "octopus security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "octopus-npm-pack-",
    spec,
    timeoutMs,
