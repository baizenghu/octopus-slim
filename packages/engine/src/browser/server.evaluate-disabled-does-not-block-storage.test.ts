  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          octopus: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OCTOPUS_GATEWAY_PORT;
    process.env.OCTOPUS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OCTOPUS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OCTOPUS_GATEWAY_PASSWORD;
