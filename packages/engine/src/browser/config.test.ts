...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("octopus");
    expect(profile?.driver).toBe("octopus");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const octopus = resolveProfile(resolved, "octopus");
    expect(octopus?.driver).toBe("octopus");
    expect(octopus?.cdpPort).toBe(18800);
    expect(octopus?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OCTOPUS_GATEWAY_PORT when unset", () => {
    withEnv({ OCTOPUS_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const octopus = resolveProfile(resolved, "octopus");
      expect(octopus?.cdpPort).toBe(19012);
      expect(octopus?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ OCTOPUS_GATEWAY_PORT: undefined }, () => {
      ...
      const octopus = resolveProfile(resolved, "octopus");
      expect(octopus?.cdpPort).toBe(19022);
      expect(octopus?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(octopus?.cdpPort).toBe(19000);
    expect(octopus?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "octopus");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        octopus: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("octopus");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to octopus profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps octopus default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("octopus");
    ...
    it("keeps octopus default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("octopus");
    ...
    it("keeps octopus default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
