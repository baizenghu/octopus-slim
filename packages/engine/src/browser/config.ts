import type { BrowserConfig, BrowserProfileConfig, OctopusConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OCTOPUS_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OCTOPUS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "octopus" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OCTOPUS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OCTOPUS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Octopus browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for octopus/octopus CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OctopusConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OCTOPUS_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_OCTOPUS_BROWSER_PROFILE_NAME]
        ? DEFAULT_OCTOPUS_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "octopus";
