...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".octopus-profile-decorated");
}
 ...
 */
export function decorateOctopusProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OCTOPUS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OCTOPUS_BROWSER_COLOR).toUpperCase();
