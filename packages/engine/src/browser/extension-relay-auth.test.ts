...
import {
  probeAuthenticatedOctopusRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedOctopusRelay({
    baseUrl,
    relayAuthHeader: "x-octopus-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated octopus relay probe responses", async () => {
