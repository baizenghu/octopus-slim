...
const { stopOctopusChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopOctopusChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopOctopusChrome: stopOctopusChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "octopus", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopOctopusChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["octopus", "chrome"]);
    ...
    expect(stopMap.octopus).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopOctopusChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("octopus browser stop failed: Error: oops");
  });
});
