...
function localOctopusProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "octopus",
...
function createLocalOctopusResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localOctopusProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveOctopusUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "octopus-reset-"));
    const profileDir = path.join(tempRoot, "octopus");
    ...
    const ops = createLocalOctopusResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveOctopusUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "octopus-reset-no-own-"));
    const profileDir = path.join(tempRoot, "octopus");
    fs.mkdirSync(profileDir, { recursive: true });
