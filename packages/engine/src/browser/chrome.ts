...
export function resolveOctopusUserDataDir(profileName = DEFAULT_OCTOPUS_BROWSER_PROFILE_NAME) {
...
export async function launchOctopusChrome(
  ...
  const userDataDir = resolveOctopusUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_OCTOPUS_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOctopusProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 octopus browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`octopus browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`octopus browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 octopus browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopOctopusChrome(
