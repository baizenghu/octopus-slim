...
const RELAY_TOKEN_CONTEXT = "octopus-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const OCTOPUS_RELAY_BROWSER = "Octopus/extension-relay";
...
async function resolveGatewayAuthToken(): Promise<string | null> {
  const envToken =
    process.env.OCTOPUS_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
    ...
    if (tokenRef) {
      ...
      throw new SecretRefUnavailableError(
        `extension relay requires a resolved gateway token, but gateway.auth.token SecretRef is unavailable (${refLabel}). Set OCTOPUS_GATEWAY_TOKEN or resolve your secret provider.`,
      );
    }
...
export async function resolveRelayAcceptedTokensForPort(port: number): Promise<string[]> {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or OCTOPUS_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedOctopusRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === OCTOPUS_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
