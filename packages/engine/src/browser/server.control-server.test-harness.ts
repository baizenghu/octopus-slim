...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.OCTOPUS_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/octopus" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "octopus",
        profiles: {
          octopus: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOctopusChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveOctopusUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopOctopusChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.OCTOPUS_GATEWAY_PORT;
  process.env.OCTOPUS_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.OCTOPUS_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.OCTOPUS_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.OCTOPUS_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.OCTOPUS_GATEWAY_PASSWORD;
  } else {
    process.env.OCTOPUS_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
