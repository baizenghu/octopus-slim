...
import { createOctopusTools } from "../agents/octopus-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-octopus-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-octopus-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-octopus-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-octopus-thread-id")?.trim() || undefined;
  ...
  const allTools = createOctopusTools({
  ...
  const defaultGatewayDeny: string[] = DEFAULT_GATEWAY_HTTP_TOOL_DENY.filter(
    (name: any) => !gatewayToolsCfg?.allow?.includes(name),
  );
