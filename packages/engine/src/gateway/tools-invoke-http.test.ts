...
let lastCreateOctopusToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/octopus-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateOctopusToolsContext?.agentTo,
          agentThreadId: lastCreateOctopusToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createOctopusTools: (ctx: Record<string, unknown>) => {
      lastCreateOctopusToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateOctopusToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateOctopusToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-octopus-message-to": "channel:24514",
        "x-octopus-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
