...
import { resolveOctopusAgentDir } from "../agents/agent-paths.js";
...
import { ensureOctopusModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, OctopusConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.OCTOPUS_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OCTOPUS_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OCTOPUS_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OCTOPUS_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.OCTOPUS_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.OCTOPUS_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.OCTOPUS_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<OctopusConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.OCTOPUS_CONFIG_PATH,
    token: process.env.OCTOPUS_GATEWAY_TOKEN,
    skipChannels: process.env.OCTOPUS_SKIP_CHANNELS,
    skipGmail: process.env.OCTOPUS_SKIP_GMAIL_WATCHER,
    skipCron: process.env.OCTOPUS_SKIP_CRON,
    skipCanvas: process.env.OCTOPUS_SKIP_CANVAS_HOST,
    agentDir: process.env.OCTOPUS_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.OCTOPUS_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.OCTOPUS_SKIP_CHANNELS = "1";
  process.env.OCTOPUS_SKIP_GMAIL_WATCHER = "1";
  process.env.OCTOPUS_SKIP_CRON = "1";
  process.env.OCTOPUS_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.OCTOPUS_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveOctopusAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-live-state-"));
  process.env.OCTOPUS_STATE_DIR = tempStateDir;
  ...
  process.env.OCTOPUS_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.octopus-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOctopusAgentDir();
  const sanitizedCfg: OctopusConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-live-"));
  const tempConfigPath = path.join(tempDir, "octopus.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.OCTOPUS_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "Octopus live tool probe (local, safe): " +
    ...
    process.env.OCTOPUS_CONFIG_PATH = previous.configPath;
    process.env.OCTOPUS_GATEWAY_TOKEN = previous.token;
    process.env.OCTOPUS_SKIP_CHANNELS = previous.skipChannels;
    process.env.OCTOPUS_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.OCTOPUS_SKIP_CRON = previous.skipCron;
    process.env.OCTOPUS_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.OCTOPUS_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OCTOPUS_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureOctopusModelsJson(cfg);
      ...
      const rawModels = process.env.OCTOPUS_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via OCTOPUS_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.octopus-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
