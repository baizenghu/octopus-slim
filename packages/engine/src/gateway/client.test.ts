...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("octopus doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with OCTOPUS_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with OCTOPUS_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.OCTOPUS_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://octopus-gateway.ai:18789",
      onConnectError,
    });
