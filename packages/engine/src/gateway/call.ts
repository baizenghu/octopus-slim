...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.OCTOPUS_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.OCTOPUS_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set OCTOPUS_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: octopus doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__OCTOPUS_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
