...
export function resolveHooksConfig(cfg: OctopusConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OctopusConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-octopus-token"] === "string"
      ? req.headers["x-octopus-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
