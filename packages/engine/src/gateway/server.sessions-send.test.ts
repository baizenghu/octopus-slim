...
const { createOctopusTools } = await import("../agents/octopus-tools.js");
...
type SessionSendTool = ReturnType<typeof createOctopusTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOctopusTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OCTOPUS_GATEWAY_PORT", "OCTOPUS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OCTOPUS_GATEWAY_PORT = String(gatewayPort);
  process.env.OCTOPUS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OCTOPUS_CONFIG_PATH missing in gateway test environment");
      }
