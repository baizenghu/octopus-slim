...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OCTOPUS_STATE_DIR",
  "OCTOPUS_CONFIG_PATH",
  "OCTOPUS_SKIP_BROWSER_CONTROL_SERVER",
  "OCTOPUS_SKIP_GMAIL_WATCHER",
  "OCTOPUS_SKIP_CANVAS_HOST",
  "OCTOPUS_BUNDLED_PLUGINS_DIR",
  "OCTOPUS_SKIP_CHANNELS",
  "OCTOPUS_SKIP_PROVIDERS",
  "OCTOPUS_SKIP_CRON",
  "OCTOPUS_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OCTOPUS_STATE_DIR = path.join(tempHome, ".octopus");
  delete process.env.OCTOPUS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OCTOPUS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OCTOPUS_SKIP_GMAIL_WATCHER = "1";
  process.env.OCTOPUS_SKIP_CANVAS_HOST = "1";
  process.env.OCTOPUS_SKIP_CHANNELS = "1";
  process.env.OCTOPUS_SKIP_PROVIDERS = "1";
  process.env.OCTOPUS_SKIP_CRON = "1";
  process.env.OCTOPUS_TEST_MINIMAL_GATEWAY = "1";
  process.env.OCTOPUS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "octopus-test-no-bundled-extensions")
    : "octopus-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".octopus-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".octopus-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__octopusTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__octopusTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OCTOPUS_GATEWAY_TOKEN"]);
  const prev = process.env.OCTOPUS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OCTOPUS_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.OCTOPUS_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
