  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set OCTOPUS_GATEWAY_TOKEN/OCTOPUS_GATEWAY_PASSWORD, pass explicit --token/--password,",
 ...
 * This prevents literal placeholder strings like `${OCTOPUS_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.OCTOPUS_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.OCTOPUS_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  const localTokenPrecedence =
    ...
    (env.OCTOPUS_SERVICE_KIND === "gateway" ? "config-first" : "env-first");
