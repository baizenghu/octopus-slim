...
`;
    ...
    expect(parsed.octopus.emoji).toBe("💾");
    expect(parsed.octopus.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.octopus.emoji).toBe("📝");
    expect(parsed.octopus.events).toEqual(["command"]);
    expect(parsed.octopus.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.octopus.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"octopus": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"octopus": {"events": ["test"]}}');
  });
...
describe("resolveOctopusMetadata", () => {
  it("extracts octopus metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOctopusMetadata(frontmatter);
  ...
  it("returns undefined when octopus key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        octopus: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Octopus" },
            { id: "npm", kind: "npm", package: "@octopus/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@octopus/hook");
  });
...
homepage: https://docs.octopus.ai/automation/hooks#session-memory
metadata:
  {
    "octopus":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Octopus" }],
      },
  }
---
...
`;
    ...
    const octopus = resolveOctopusMetadata(frontmatter);
    expect(octopus).toBeDefined();
    expect(octopus?.emoji).toBe("💾");
    expect(octopus?.events).toEqual(["command:new", "command:reset"]);
    expect(octopus?.requires?.config).toEqual(["workspace.dir"]);
    expect(octopus?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const octopus = resolveOctopusMetadata(frontmatter);
    expect(octopus?.emoji).toBe("disk");
    expect(octopus?.events).toEqual(["command:new"]);
  });
});
