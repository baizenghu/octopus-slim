...
declare const __OCTOPUS_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "octopus";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["OCTOPUS_VERSION"],
      runtimeVersion,
      env["OCTOPUS_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Octopus version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __OCTOPUS_VERSION__ === "string" ? __OCTOPUS_VERSION__ : undefined,
  bundledVersion: process.env.OCTOPUS_BUNDLED_VERSION,
});
