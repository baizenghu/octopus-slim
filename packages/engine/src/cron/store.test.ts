...
const { makeStorePath } = createCronStoreHarness({ prefix: "octopus-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OCTOPUS_HOME for tilde expansion", () => {
    vi.stubEnv("OCTOPUS_HOME", "/srv/octopus-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/octopus-home", "cron", "jobs.json"));
  });
});
