...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "octopus-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".octopus", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OctopusConfig> = {},
): OctopusConfig {
  const base: OctopusConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<OctopusConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<OctopusConfig>,
