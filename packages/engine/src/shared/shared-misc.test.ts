...
describe("shared/frontmatter", () => {
  ...
  test("resolveOctopusManifestBlock parses JSON5 metadata and picks octopus block", () => {
    const frontmatter = {
      metadata: "{ octopus: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveOctopusManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveOctopusManifestBlock returns undefined for invalid input", () => {
    expect(resolveOctopusManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveOctopusManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveOctopusManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
