...
const OCTOPUS_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of OCTOPUS_COMPILED_SECRET_TARGETS) {
...
const OCTOPUS_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: OCTOPUS_COMPILED_SECRET_TARGETS,
    entriesById: OCTOPUS_TARGETS_BY_ID,
  });
