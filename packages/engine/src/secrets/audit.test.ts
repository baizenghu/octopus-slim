...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "octopus-secrets-audit-"));
  const stateDir = path.join(rootDir, ".octopus");
...
describe("secrets audit", () => {
  ...
  it("does not flag non-sensitive routing headers in octopus config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
