// Re-export from original utils.ts — provides core utility functions
// used across the engine (ensureDir, clamp, CONFIG_DIR, etc.)
...
export function resolveConfigDir(
  ...
  const override = env.OCTOPUS_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".octopus");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OCTOPUS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OCTOPUS_HOME" };
  }
  return { home, prefix: "~" };
}
