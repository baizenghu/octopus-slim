...
function telegramCfg(): OctopusConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OctopusConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OctopusConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OctopusConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OctopusConfig;
...
function createSignalAccountOverrideCfg(): OctopusConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OctopusConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OctopusConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OctopusConfig,
        accountId: undefined,
        params: {
