...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OctopusConfig, policy: ChannelAccessPolicy) => OctopusConfig;
  resolveAllowlist: (params: { cfg: OctopusConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OctopusConfig; resolved: TResolved }) => OctopusConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OctopusConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OctopusConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OctopusConfig, policy: ChannelAccessPolicy): OctopusConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OctopusConfig, policy: ChannelAccessPolicy): OctopusConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OctopusConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OctopusConfig; resolved: string[] }) => {
      calls.push("apply");
