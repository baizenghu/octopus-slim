import type { AnyAgentTool, OctopusPluginApi } from "octopus/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: OctopusPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<OctopusPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
