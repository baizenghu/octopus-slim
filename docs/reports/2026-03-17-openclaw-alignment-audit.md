...
> 审计结论：**P0×4、P1×14、P2×16、P3×8 = 42 个问题**，死代码约 1800 行，重复代码约 800 行
...
#### P0-2：API Key 明文存储在 git 跟踪文件中 🆕来自对方报告

**现象**：DeepSeek API Key、Jina API Key、MySQL 连接字符串（含密码）等敏感信息以明文存储在 `octopus.json` 中，且该文件被 git 跟踪。配置备份（`config-backups/`）也包含明文密钥。

**建议**：使用 OpenClaw 原生的 SecretRef 系统（`{ "$secretRef": { "source": "env", "id": "DEEPSEEK_API_KEY" } }`），或至少迁移到环境变量引用。

#### P0-3：extraSystemPrompt 与 TOOLS.md 双重注入 🆕来自对方报告
...
#### P1-1：企业技能在宿主机 uid=1000 执行 ✅双方确认

`run_skill` 插件工具通过 `child_process.spawn()` 直接在宿主机执行 Python/Node 脚本，身份为 baizh（uid=1000），理论上可访问宿主机所有文件。个人 Skill 反而在 Docker 容器中执行（`--network=none`），安全级别更高——与直觉相反。
...
### 6.2 存在分歧的能力：`skills.load.extraDirs`

- **团队 A 结论**：已正确利用（CLAUDE.md 2026-02-26 迁移记录标注完成）
- **团队 B 结论**：octopus.json 中无 `skills` 配置块，原生引擎对企业 Skill 完全不可见，SkillsInfo.ts 手动读 SKILL.md 构建提示与原生提示注入重复

**待验证**：检查当前 `octopus.json` 中是否有 `skills.load.extraDirs` 配置。如果缺失，说明 2026-03-16 覆盖事件中一同丢失。
...
| 2 | API Key 明文存储在 git 跟踪文件中 | octopus.json | 🆕对方 |
| 3 | extraSystemPrompt 与 TOOLS.md 双重注入 | chat.ts | 🆕对方 |
| 4 | 飞书 `/bind` 密码明文传输 | IM 集成 | 团队 A |
