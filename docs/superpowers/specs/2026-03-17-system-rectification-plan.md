...
> 基于 OpenClaw 官方文档学习 + 两个独立团队深度审计交叉验证，制定的分阶段整改方案。
> 审计报告详见 `docs/reports/2026-03-17-openclaw-alignment-audit.md`（合并版，42 个问题）
...
1. **死代码残留**：约 1800 行已废弃但未清理的代码（含 SkillTools.ts）
2. **安全配置缺失**：Docker sandbox 引擎配置缺失、API Key 明文在 git 中、HMAC 默认密钥
3. **代码重复严重**：chat.ts ~1500 行职责过重、enterprise-mcp 插件 4 个重复 execute 函数、个人 MCP CRUD 双入口、extraSystemPrompt 与 TOOLS.md 双重注入
4. **性能问题**：每条消息 7+ 次 DB/文件查询（无缓存）、启动时 Agent 串行同步、委派结果暴力轮询
5. **配置管理混乱**：SystemConfigPage 与原生 Control UI 双入口冲突、configApplyFull 全量替换有丢失风险
6. **原生能力未充分利用**：工具 deny 列表、IDENTITY.md creature/vibe 字段、技能状态双写同步、config.patch RPC 等
...
```
阶段 1（清理 + 安全 + 快速修复）
  ├── 1.8 chat.ts 快速修复 ──→ 阶段 2.1 chat.ts 瘦身拆分（先修 bug 再拆分，避免搬运 bug）
  ├── 1.9.1 删除 Agent 清理 config ──→ 阶段 2.4 config 写入合并（合并函数需正确处理删除场景）
...
| `apps/server/src/services/SkillTools.ts` | ~100 | 定义了未使用的 executeSkillToolCall，实际由 plugin run_skill 处理 |
...
**问题**：`AUDIT_HMAC_KEY` 默认值为 `'default-audit-key-change-me'`，生产环境未设环境变量则签名链形同虚设。

**修复**：enterprise-audit 插件启动时检查 `AUDIT_HMAC_KEY`，如果是默认值则打印 `[WARN] AUDIT_HMAC_KEY is using default value, audit signature chain is NOT secure` 日志。

### 1.3 恢复 `skills.load.extraDirs` 配置 🆕

**问题**：对方团队通过 grep 确认当前 `octopus.json` 中**没有** `skills` 配置块。CLAUDE.md 记录 2026-02-26 已迁移到 `skills.load.extraDirs`，但配置在 2026-03-16 vitest 覆盖事件中丢失。导致原生引擎对企业 Skill 完全不可见，`SkillsInfo.ts` 手动重建了原生已有的提示注入功能。

**修复**：在 `octopus.json` 中恢复：
```json
{
  "skills": {
    "load": {
...
**验证**：重启后通过原生 CLI `openclaw skills list` 确认企业技能可见。

### 1.4 企业 MCP 环境变量泄露修复 🆕

**问题**：企业 MCP 的 `spawn` 调用使用 `env: { ...process.env, ...mergedEnv }`，继承了完整宿主环境变量，包括 `DATABASE_URL`、`JWT_SECRET`、`DEEPSEEK_API_KEY` 等敏感信息。恶意或有缺陷的 MCP Server 进程可以读取所有密钥。

**修复**：企业 MCP spawn 时过滤环境变量，只传递必要的：
```typescript
// 当前: env: { ...process.env, ...mergedEnv }
// 修复:
env: {
  PATH: process.env.PATH,
  HOME: '/tmp',
  NODE_ENV: process.env.NODE_ENV,
  ...mergedEnv  // 仅管理员显式配置的变量
}
```

### 1.5 `start.sh` 配置完整性校验 🆕

**问题**：2026-03-16 vitest worker 覆盖 `octopus.json` 导致多个关键配置丢失（sandbox、skills.load.extraDirs）。当前只有备份机制，无主动检测。

**修复**：在 `start.sh` 启动 native gateway 前，检查 `octopus.json` 中必要字段：
```bash
# 检查关键配置是否存在
python3 -c "
import json
with open('.octopus-state/octopus.json') as f:
...
for path, expected in required:
    # 简单路径检查
    ...
" || echo "[WARN] octopus.json missing critical fields, check config-backups/"
```
...
**1.8.5 tools-cache.json 位置迁移** 🆕

当前写入 `plugins/mcp/` 目录下，污染版本控制。
修复：迁移到 `.octopus-state/tools-cache.json`，与其他运行时状态文件统一管理。读取路径同步更新。
...
grep -r 'SkillTools' apps/server/src/     # 应无 import
grep -r 'McpPage\|SkillsPage' apps/console/src/  # 应无 import

# skills 配置恢复验证
grep -q 'extraDirs' .octopus-state/octopus.json && echo "OK" || echo "MISSING"

# sandbox 验证
# agent 执行 whoami 确认输出 sandbox（uid=2000）

# 企业 MCP 环境变量验证
# 在企业 MCP Server 进程中执行 env 命令，确认不含 DATABASE_URL/JWT_SECRET 等
...
# 检查 enterprise-audit 启动日志中 HMAC 密钥警告
```
...
**前置依赖**：阶段 1 全部完成（特别是 1.8 chat.ts 快速修复和 1.9.1 删除 Agent 清理 config）。
...
### 2.1.1 消除 extraSystemPrompt 与 TOOLS.md 双重注入 🆕

**问题**：`buildEnterpriseSystemPrompt()` 中的 `buildMCPToolsSection()` 构建 MCP 工具说明注入到 `extraSystemPrompt`，同时 `syncToolsMd()` 已把相同信息写入 TOOLS.md，原生引擎也会注入 TOOLS.md + tool catalog。同一信息被注入了两次，浪费 context token。

**修复**：
- 删除 `buildMCPToolsSection()`，MCP 工具说明由 TOOLS.md + 引擎 tool catalog 承载
- `extraSystemPrompt` 只保留动态信息（当前用户名、workspace 路径、安全约束）
- 静态内容（通用约束、操作规范）移入 SOUL.md，只在 agent 创建/更新时同步
...
### 2.5 中文 agentName 转换冲突修复 🆕

**问题**：`userAgentId()` 将非 `[a-z0-9_-]` 字符替换为 `_`，多个中文名 agent 可能产生相同 ID（如"财务助手"和"技术助手"都变成 `ent_user_____`）。

**修复**：中文字符使用 pinyin 转换（如 `pinyin("财务助手")` → `caiwuzhushou`），保证唯一性。如无 pinyin 库，改用 hash 后缀（如 `ent_user_agent_a1b2c3`）。
...
### 3.2 减少 `configApplyFull` 使用 + 评估 `config.patch` RPC 🆕补充
...
**新增评估项**：OpenClaw 原生还提供 `config.patch` RPC（RFC 7396 Merge Patch 语义），可能比 `configApply`（自定义 deepMerge）更标准化。实施时应先确认 EngineAdapter 是否已封装 `config.patch`，以及其对数组字段的处理方式。

实施策略：
- 对于非数组字段（如 `tools.deny`、`skills.entries`、`plugins.entries`）：改用 `configApply` 或 `config.patch`（patch），有实际意义
...
### 3.7 个人 MCP/Skill 沙箱参数统一 🆕

**问题**：个人 MCP 和个人 Skill 的 Docker 沙箱参数不一致：
- 个人 MCP: `--memory 256m --cpus 0.5 --network octopus-internal`
- 个人 Skill: `--memory 512m --cpus 1 --network none`

**修复**：统一从配置读取沙箱参数，新增 `sandbox.personal` 配置块：
```json
{
  "sandbox": {
    "personal": {
...
| `buildEnterpriseSystemPrompt` 中的"严禁访问"文字指令 | 阶段 1.4 已激活 Docker sandbox 硬隔离，文字指令作为辅助提示保留，不构成安全边界 |
| 引入 Session Pruning / Memory Flush | 当前会话长度可控，中期再评估 |
| 使用 `$include` 配置拆分 | 当前用户量少，并发冲突概率低 |
