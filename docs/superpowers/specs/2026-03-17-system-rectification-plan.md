...
```
阶段 1（清理 + 安全 + 快速修复）
  ├── 1.5 chat.ts 快速修复 ──→ 阶段 2.1 chat.ts 瘦身拆分（先修 bug 再拆分，避免搬运 bug）
  ├── 1.6.1 删除 Agent 清理 config ──→ 阶段 2.4 config 写入合并（合并函数需正确处理删除场景）
...
**前置依赖**：阶段 1 全部完成（特别是 1.5 chat.ts 快速修复和 1.6.1 删除 Agent 清理 config）。
