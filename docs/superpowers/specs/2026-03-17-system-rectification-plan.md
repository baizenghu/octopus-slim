...
```
阶段 1（清理 + 安全 + 快速修复）
  ├── 1.6 chat.ts 快速修复 ──→ 阶段 2.1 chat.ts 瘦身拆分（先修 bug 再拆分，避免搬运 bug）
  ├── 1.7.1 删除 Agent 清理 config ──→ 阶段 2.4 config 写入合并（合并函数需正确处理删除场景）
...
### 1.2 API Key 明文迁移到环境变量 🆕

**问题**：DeepSeek API Key、Jina API Key、MySQL 连接字符串（含密码）等敏感信息以明文存储在 `octopus.json` 中，且该文件被 git 跟踪。配置备份（`config-backups/`）也包含明文密钥。

**修复**：将敏感配置迁移到环境变量引用：
- `octopus.json` 中 `apiKey` 字段改为 `"${DEEPSEEK_API_KEY}"` 格式（OpenClaw 原生支持 `[A-Z_][A-Z0-9_]*` 格式的环境变量引用）
- `.env` 文件中添加对应的环境变量（`.env` 已在 `.gitignore` 中）
- 备份机制同步调整：`config-backups/` 中的备份不再包含明文密钥

**验证**：重启后确认模型调用正常（API Key 从环境变量正确注入）。
...
# API Key 迁移验证
grep -r 'apiKey.*[a-zA-Z0-9]\{20,\}' .octopus-state/octopus.json  # 应无明文密钥
# 重启后确认模型调用正常（API Key 从 .env 正确注入）
...
**前置依赖**：阶段 1 全部完成（特别是 1.6 chat.ts 快速修复和 1.7.1 删除 Agent 清理 config）。
