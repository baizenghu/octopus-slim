...
def phase_leader(config, tmpl_data, topic, context, num_workers):
    ...
    for attempt in range(2):  # 最多重试 1 次
        try:
            ...
            dims = parse_json_response(result)
            elapsed = round(time.time() - t0, 1)
            phase_info = {
                "name": "任务拆分",
                "role": leader.get("role", "组长"),
                "duration_sec": elapsed,
                "output_summary": f"将课题拆分为 {len(dims)} 个维度: {', '.join(d['name'] for d in dims)}",
            }
            return dims, phase_info
            ...
            phase_info = {
                ...
                "role": leader.get("role", "组长"),
                "duration_sec": elapsed,
                "output_summary": f"降级拆分为 {len(dims)} 个维度: {', '.join(d['name'] for d in dims)}",
            }
            return dims, phase_info

def phase_workers(config, tmpl_data, dimensions, topic, context):
    """Phase 2: Workers 并行调研
    返回 (findings, phase_info)"""
    workers = tmpl_data.get("workers", [])
    findings = [None] * len(dimensions)
    worker_infos = [None] * len(dimensions)
    max_concurrent = config["params"].get("max_concurrent_workers", 5)

    # 构建原始材料引用块，传递给每个 worker
    source_block = f"\n\n=== 原始材料（必须基于此内容，严禁编造） ===\n{topic}\n"
    if context:
        source_block += f"\n补充背景：{context}\n"
    source_block += "=== 原始材料结束 ===\n"
    ...
    def run_worker(i, dim):
        role = workers[i]["role"] if i < len(workers) else "研究员"
        if i < len(workers):
    ...
    with ThreadPoolExecutor(max_workers=min(len(dimensions), max_concurrent)) as pool:
        ...
        for future in as_completed(future_to_idx):
            ...
            try:
                result, role, elapsed = future.result()
                # 质量校验：过短的结果标记警告
                ...
                worker_infos[idx] = {
                    ...
                    "summary": result.strip()[:80] + ("..." if len(result.strip()) > 80 else ""),
                }
            except Exception as e:
                findings[idx] = f"[此维度调研失败: {e}]"
                worker_infos[idx] = {
                    "role": workers[idx]["role"] if idx < len(workers) else "研究员",
                    "dimension": dimensions[idx]["name"],
                    "duration_sec": 0,
                    "output_chars": 0,
                    "summary": f"调研失败: {e}",
    ...
    phase_info = {
        ...
        "total_duration_sec": round(time.time() - phase_t0, 1),
    }
    return findings, phase_info

def phase_editor(config, tmpl_data, topic, context, dimensions, findings):
    """Phase 3: Editor 汇总合成（含降级）
    返回 (report, phase_info)"""
    ...
    try:
        ...
        phase_info = {
            "name": "报告整合",
            "role": editor.get("role", "编辑"),
            "duration_sec": elapsed,
            "output_chars": len(report),
        }
        return report, phase_info
    except Exception as e:
        # 降级：直接拼接 Worker 成果
...
def main():
    ...
    parser.add_argument("--source-file", default="", help="原始参考材料文件路径（workspace 内）")
    args = parser.parse_args()

    # 如果指定了 source-file，读取文件内容追加到 context
    if args.source_file:
        ...
        if source_path.exists():
            ...
            args.context = f"{args.context}\n\n=== 参考材料内容 ===\n{source_content}" if args.context else source_content
            print(f"[ClawTeam] 已加载参考材料: {source_path} ({len(source_content)} 字符)", file=sys.stderr)
        else:
            print(f"[ClawTeam] 警告: 参考材料文件不存在: {source_path}", file=sys.stderr)
    ...
    # Phase 1
    dimensions, leader_info = phase_leader(config, tmpl_data, args.topic, args.context, num_workers)
    ...
    findings, workers_info = phase_workers(config, tmpl_data, dimensions, args.topic, args.context)
    ...
    report, editor_info = phase_editor(config, tmpl_data, args.topic, args.context, dimensions, findings)

    total_duration = round(time.time() - total_t0, 1)

    # 输出文件
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    filename = f"team-{template_name}-{timestamp}.md"
    ...
    result = {
        ...
        "team_process": {
            "title": args.topic[:60] + ("..." if len(args.topic) > 60 else ""),
